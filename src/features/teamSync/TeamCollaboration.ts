...
export class TeamCollaboration {
  ...
  async calculateOptimalTimes(
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      if (availableMembers.length >= 2) {
        // At least 2 members needed for collaboration
        ...
        if (validBlock) {
          timeSlots.push({
            ...
            score: this.calculateTimeSlotScore(
              hour,
   ...
   */
  async checkCollaborationSuitability(
    members: TeamMemberStatus[]
  ): Promise<boolean> {
    const activeMembers = members.filter(
      (m) => m.status === "available" || m.status === "focusing"
    );
  ...
  private calculateSessionFrequency(sessions: any[]): number {
    ...
    const recentSessions = sessions.filter(
      (s) => now - new Date(s.created_at).getTime() <= weekMs
    );
