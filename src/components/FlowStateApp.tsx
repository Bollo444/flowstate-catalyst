...
export const FlowStateApp: React.FC<FlowStateAppProps> = ({
  initialTeamId,
}) => {
  ...
  useEffect(() => {
    const initializeFlowState = async () => {
      ...
      const initialState = FlowCalculator.createFlowState(
        user.id,
        initialMetrics
      );
      updateFlowState({
      ...
      <AnimatePresence>
        ...
        {flowState.userId && (
          <motion.div
            ...
            variants={flowAnimations.enter}
          >
            <ActiveTasks userId={flowState.userId} teamId={teamId} />
          </motion.div>
        )}
        ...
        >
          {teamId && (
            <TeamFlowSync teamId={teamId} currentUserId={flowState.userId} />
          )}
        </motion.div>
      </AnimatePresence>
    </motion.div>
