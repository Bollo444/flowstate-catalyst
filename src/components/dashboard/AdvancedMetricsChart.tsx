...
export const AdvancedMetricsChart: React.FC<AdvancedMetricsChartProps> = ({
  ...
  const handleZoom = useCallback((event: Event, newValue: number | number[]) => {
    ...
    <div className={styles.advancedChartContainer}>
      ...
      <ResponsiveContainer width="100%" height={400}>
        <ComposedChart data={data}>
          ...
          <Legend 
            onClick={(e) => setFocusedMetric(e?.dataKey as string)}
          />
          ...
          {selectedMetrics.map(metric => {
            ...
            return metric === 'sessionCount' ? (
              <Bar
                ...
                fill={config.color}
                yAxisId="right"
                opacity={focusedMetric ? 
                  focusedMetric === metric ? 1 : 0.3 : 1
                }
              />
            ) : (
              <Line
      ...
      <div className={styles.chartInsights}>
        {generateChartInsights(data, selectedMetrics).map((insight, index) => (
          <p key={index} className={styles.insight}>{insight}</p>
        ))}
      </div>
    </div>
  );
