...
class FlowSystemValidator {
  ...
  private async testRealtimeDataFlow(): Promise<DataFlowTestResult> {
    ...
    try {
      ...
      throw new Error(`Real-time data flow test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private determineHealthStatus(latency: number, errorRate: number): DataFlowTestResult['status'] {
