...
export const useTaskRouter = (
  ...
  const user = useUser();
  const { pendingTasks, flowOptimizedTasks, updateTaskProgress } = useTaskFlow(
    user?.id || ""
  );
  ...
  useEffect(() => {
    ...
    const shouldRoute =
      ...
      !state.lastUpdate ||
      new Date().getTime() - new Date(state.lastUpdate).getTime() >
        30 * 60 * 1000; // Re-route every 30 minutes
        ...
        const bestHour = hours.reduce(
          (best, hour) => {
            const isPreferredHour =
              userFlowState.focusPreferences?.preferredFocusHours.includes(
                hour
              );
